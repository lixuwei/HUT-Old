package com.hut.domain;

/**
 * TTeacher entity. @author MyEclipse Persistence Tools
 */

/*
 * 老师基础信息表
 */
public class Teacher implements java.io.Serializable {

	// Fields
	private String lsbh;
	private String xm;
	private String zgh;
	private String zplj;
	private int    xb ;
	private String mzdm;
	private String csrq;
	private String zzmmdm;
	private String xydm;
	private String xdm;
	private String  lxdh;
	private String email;
	private String  byxxdm;
	private String byzydm;
	private String  xldm;
	private String  xwdm;
	private String  zcdm;
	private String zydm;
	private String zw;
	private String xkfx;
	private String sfzh;
	private String jszgzh;
	private String zjjszgzh;
	private String jxzlpj;
	private String jsjj;

	
	public String getZydm() {
		return zydm;
	}
	public void setZydm(String zydm) {
		this.zydm = zydm;
	}
	public String getJszgzh() {
		return jszgzh;
	}
	public void setJszgzh(String jszgzh) {
		this.jszgzh = jszgzh;
	}
	public String getZjjszgzh() {
		return zjjszgzh;
	}
	public void setZjjszgzh(String zjjszgzh) {
		this.zjjszgzh = zjjszgzh;
	}
	public String getJxzlpj() {
		return jxzlpj;
	}
	public void setJxzlpj(String jxzlpj) {
		this.jxzlpj = jxzlpj;
	}
	public String getJsjj() {
		return jsjj;
	}
	public void setJsjj(String jsjj) {
		this.jsjj = jsjj;
	}
	public String getLsbh() {
		return lsbh;
	}
	public void setLsbh(String lsbh) {
		this.lsbh = lsbh;
	}
	public String getXm() {
		return xm;
	}
	public void setXm(String xm) {
		this.xm = xm;
	}
	public String getZgh() {
		return zgh;
	}
	public void setZgh(String zgh) {
		this.zgh = zgh;
	}
	public String getZplj() {
		return zplj;
	}
	public void setZplj(String zplj) {
		this.zplj = zplj;
	}
	public int getXb() {
		return xb;
	}
	public void setXb(int xb) {
		this.xb = xb;
	}
	public String getMzdm() {
		return mzdm;
	}
	public void setMzdm(String mzdm) {
		this.mzdm = mzdm;
	}
	public String getCsrq() {
		return csrq;
	}
	public void setCsrq(String csrq) {
		this.csrq = csrq;
	}
	public String getZzmmdm() {
		return zzmmdm;
	}
	public void setZzmmdm(String zzmmdm) {
		this.zzmmdm = zzmmdm;
	}
	public String getXydm() {
		return xydm;
	}
	public void setXydm(String xydm) {
		this.xydm = xydm;
	}
	public String getXdm() {
		return xdm;
	}
	public void setXdm(String xdm) {
		this.xdm = xdm;
	}
	public String getLxdh() {
		return lxdh;
	}
	public void setLxdh(String lxdh) {
		this.lxdh = lxdh;
	}
	public String getEmail() {
		return email;
	}
	public void setEmail(String email) {
		this.email = email;
	}
	public String getByxxdm() {
		return byxxdm;
	}
	public void setByxxdm(String byxxdm) {
		this.byxxdm = byxxdm;
	}
	public String getByzydm() {
		return byzydm;
	}
	public void setByzydm(String byzydm) {
		this.byzydm = byzydm;
	}
	public String getXldm() {
		return xldm;
	}
	public void setXldm(String xldm) {
		this.xldm = xldm;
	}
	public String getXwdm() {
		return xwdm;
	}
	public void setXwdm(String xwdm) {
		this.xwdm = xwdm;
	}
	public String getZcdm() {
		return zcdm;
	}
	public void setZcdm(String zcdm) {
		this.zcdm = zcdm;
	}
	public String getZw() {
		return zw;
	}
	public void setZw(String zw) {
		this.zw = zw;
	}
	public String getXkfx() {
		return xkfx;
	}
	public void setXkfx(String xkfx) {
		this.xkfx = xkfx;
	}
	public String getSfzh() {
		return sfzh;
	}
	public void setSfzh(String sfzh) {
		this.sfzh = sfzh;
	}

}